<template>
  <div class="container mt-5">
    <h1><img src="/images/banka.gif" style="max-width: 60px; margin-right: 30px;">Specify the number of the jar and its volume</h1>
    <div class="row">
      <div class="col-10 mx-auto">
        ...
        <div class="  mt-3 d-flex align-items-start justify-content-between">
          <div class="input-group">
            ...
            <input type="text" class="form-control" :class="{'is-invalid': $v.number1.$error}"
                   @input="$v.number1.$touch()" v-model.trim="number1">
            <div class="invalid-feedback" v-if="$v.number1.required">Field can`t be empty</div>
            <div class="invalid-feedback" v-if="$v.number1.minLength">The name must contain at least one character</div>
          ...
          <div class="input-group">
            ...
            <input type="text" class="form-control " :class="{'is-invalid': $v.value1.$error }"
                   @input="$v.value1.$touch()" v-model.number="value1">
            <!--            <div class="invalid-feedback" v-if="$v.value1.required">Поле не может быть пустым</div>-->
            <div class="invalid-feedback" v-if="$v.value1.numeric">You can only use numbers</div>
<!--            <div class="error" v-if="valueValidate">Такой объём банок уже существует</div>-->
          </div>
          {{$v.value1}}
